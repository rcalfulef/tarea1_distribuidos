...
def cliente_thread(conn,addr,sockets,max_buffer_size=1024):
    ...
    while cliente_activo:
        ...
        if "--quit--" in input_cliente: # si el cliente envia salir
        ...
        elif "--conexion--" in input_cliente:
            ...
            guardar_en_archivo(input_cliente,addr,aleatorio.getpeername())
            mensaje_a_cliente = "MENSAJE DEL CLIENTE:\n    "+input_cliente+"\n FUE ALMACENADO EN "+str(aleatorio.getpeername())+'\n'
            conn.send(mensaje_a_cliente.encode())
        
        
def recibir_input(conn,addr,addrDatanode,max_buffer_size=1024):
    input_cliente = conn.recv(max_buffer_size)
    decoded_input = input_cliente.decode()
    #resultado_guardar = guardar_en_archivo(decoded_input,addr,addrDatanode)
...
def guardar_en_archivo(input_str,addr,addrDatanode):
    file  = open("archivos/registro_server.txt","a")
    file.write("EL MENSAJE: "+ input_str +" , DEL CLIENTE " + str(addr)+ ", FUE GUARDADO EN EL DATANODE " + str(addrDatanode) + "\n")
    file.close()
...
def multicast(sockets):
    ...
    for socket in sockets:
        ...
        if "--vivo--" in respuesta:
            escribir("archivos/hearbeat_server.txt",time.ctime(time.time()) + " CONEXION ACTIVA CON EL DATANODE " + str(socket.getpeername())+"\n")
